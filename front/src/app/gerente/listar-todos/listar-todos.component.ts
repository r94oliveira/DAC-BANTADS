...
export class ListarTodosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    document.querySelector("#nome")?.addEventListener("input", this.pesquisar.bind(this,), false);
    document.querySelector("#cpf")?.addEventListener("input", this.pesquisar.bind(this,), false);
  }
    ...
    if (target.value !== "") {
      ...
      this.clienteService?.listarTodos().subscribe(clientes => {
        clientes.forEach((cliente: any) => {
          ...
          if (cliente[type].toLowerCase().includes(target.value.toLowerCase())) {
